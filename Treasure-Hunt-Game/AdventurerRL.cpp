...
Adventurer::Position AdventurerRL::checkForTreasure(const Map& map) const
{
	...
	*y = pos.get_col();
	...
	{
		int linie = map.getNumberOfRows() - 1;
		for (int col = map.getNumberOfCol() - i - 1; col < map.getNumberOfCol(); col++)
...
Adventurer::Position AdventurerRL::get_new_position(const Map& map) const
{
