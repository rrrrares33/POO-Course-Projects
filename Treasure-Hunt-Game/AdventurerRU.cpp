...
Adventurer::Position AdventurerRU::checkForTreasure(const Map& map) const
{
	...
	*y = pos.get_col();
	...
	for (int i = 1; i < map.getNumberOfCol(); i++)
	{
		int col = map.getNumberOfCol() - 1 - i;
		for (int linie = 0; linie <= i; linie++)
...
Adventurer::Position AdventurerRU::get_new_position(const Map& map) const
{
